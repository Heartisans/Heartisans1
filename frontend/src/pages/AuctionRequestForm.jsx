...
export const AuctionRequestForm = () => {
  const { user } = useUser();
  ...
  const generateAIDescription = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/generate-description", {
        ...
        additionalInfo: `Auction item request. ${form.productDescription}`
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 3. Send auction request data to backend
      const payload = {
        userId: mongoUserId,
        ...
        productWeight: form.productWeight,
        productColor: form.productColor,
        offlineVerificationDate: new Date(form.offlineVerificationDate),
      };
      
      await axios.post("http://localhost:5000/api/auction-requests", payload);
      setMsg("Auction request submitted successfully! An admin will review it.");
      
      setTimeout(() => navigate('/dashboard'), 3000);
      
    } catch (err) {
      setMsg("Failed to submit auction request.");
    }
    setLoading(false);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xl sm:text-2xl lg:text-3xl text-gray-600 max-w-3xl mx-auto">
              Submit your unique handcrafted item for auction. An admin will verify the product offline and our AI will determine the optimal price.
            </p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    required
                    className="w-full px-4 py-4 border border-gray-300 rounded-xl transition-all duration-300 text-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  />
                </div>
                
                <div className="space-y-2">
                  <label className="block text-base sm:text-lg font-medium text-gray-700">
                    Offline Verification Date *
                  </label>
                  <input
                    type="datetime-local"
                    name="offlineVerificationDate"
                    value={form.offlineVerificationDate}
                    onChange={handleChange}
                    required
                  ...
                  />
                  <p className="text-sm text-gray-500 mt-1">Select a date when an admin can verify the product physically.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-2">
                  ...
                  <input type="text" name="productMaterial" placeholder="e.g., Silk, Teak Wood" value={form.productMaterial} onChange={handleChange} required className="w-full px-4 py-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 transition-all text-lg" />
                </div>
                <div className="space-y-2">
                  <label className="block text-base sm:text-lg font-medium text-gray-700">Weight (grams)</label>
                  <input type="text" name="productWeight" placeholder="Enter weight in grams" value={form.productWeight} onChange={handleChange} className="w-full px-4 py-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 transition-all text-lg" />
                </div>
                <div className="space-y-2">
                  <label className="block text-base sm:text-lg font-medium text-gray-700">Color</label>
                  <input type="text" name="productColor" placeholder="e.g., Crimson Red" value={form.productColor} onChange={handleChange} className="w-full px-4 py-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 transition-all text-lg" />
                </div>
              </div>
              ...
              >
                {loading ? "Submitting Request..." : "Submit Auction Request"}
              </button>
            </form>
          </div>
        </div>
